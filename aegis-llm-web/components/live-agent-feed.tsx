...
function eventSummary(e: AgentEvent): string {
  const eventType = e?.event_type || (e as unknown as { event?: string })?.event || "unknown_event";
  const p = e?.payload ?? {};
  switch (eventType) {
    ...
    case "stream_end":
      return "Event stream completed";
    case "run_failed":
    case "safety_blocked":
      return String(p.message ?? p.error ?? eventType);
    default:
      return typeof eventType === "string" ? eventType.replace(/_/g, " ") : "Event received";
  }
}
    ...
    <div className="flex h-[420px] flex-col overflow-hidden rounded-lg border">
      ...
      <div className="flex-1 space-y-0 overflow-y-auto p-3 font-mono text-[12px] leading-relaxed">
        {events.length === 0 ? (
          ...
          events.map((e, i) => {
            const agentKey = (e.agent || "orchestrator").toLowerCase();
            const meta = AGENT_META[agentKey] ?? { label: e.agent || "Agent", icon: FlaskConical, color: "text-muted-foreground" };
